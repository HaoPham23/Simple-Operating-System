...
#include "sched.h"
#include "loader.h"
//#include "mm.h"			//include this from part 2
...
static struct ld_args{
	...
	unsigned long * prio;
	/* in part 2, we define prio like this	
	#ifdef MLQ_SCHED
	unsigned long * prio;
	#endif
	*/
...
static void * cpu_routine(void * args) {
	...
	/* In part 2, we use this struct pcb_t* proc instead
...
/*function read_config has lots of changes
static void read_config(const char * path) {
	FILE * file;
	if ((file = fopen(path, "r")) == NULL) {
		printf("Cannot find configure file at %s\n", path);
